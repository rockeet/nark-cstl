/* vim: set tabstop=4 : */
#ifndef __nark_trb_cxx_h__
#define __nark_trb_cxx_h__
...
template< class KeyPart, class Whole=KeyPart
		, int KeyOffset = 0
		, trb_func_compare KeyCompare = &nark_trb_compare_less_tag
...
//			const int NodeSize = DataOffset + sizeof(Whole);
			...
			trbxx_vtab_init(this
					, nark_get_field_type((KeyPart*)0)
					, KeyCompare == &nark_trb_compare_less_tag ? NULL : KeyCompare
					);
		}
	};
...
protected:
	...
	static void check_length(size_t keylen) {
		if (nark_unlikely(keylen > keylenmax)) {
...
public:
	...
	struct my_vtab : public trb_vtab_cxx_base<Data>
	{
		...
		if (boost::has_trivial_copy<Data>::value)
			...
			trbxx_vtab_init(this
					, nark_get_field_type((nark_inline_cstr_tag*)0)
					, KeyCompare == &nark_trb_compare_less_tag ? NULL : KeyCompare
					);
		}
	};
