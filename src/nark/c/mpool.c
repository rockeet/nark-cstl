...
{
	if (nark_unlikely(size > sfmp->fmp.cell_size)) {
...
void mpool_init(struct mpool* mp)
{
	...
	if (NULL == mp->fixed) {
		fprintf(stderr, "fatal: nark.mpool_init[max_cell_size="FZD", size="FZD"]\n"
...
void mpool_destroy(struct mpool* mp)
{
	...
	if (mp->big_flags & FEBIRD_MPOOL_ALLOW_BIG_BLOCK)
	{
		if (mp->big_flags & FEBIRD_MPOOL_AUTO_FREE_BIG)
		{
			...
			{
				fprintf(stderr
					, "fatal: nark.mpool_destroy: bad track list, big_blocks="FZD", i="FZD"\n"
					, mp->big_blocks, i
					);
			}
		} else {
...
{
	assert(size > 0);
	if (nark_likely(size <= mp->max_cell_size)) {
