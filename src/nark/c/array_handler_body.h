...
static const array_handler CAT_TOKEN2(_S_tab_, NARK_FUNC_NAME)[] =
{
#define MAKE_FUNC_NAME(tev) CAT_TOKEN5(NARK_FUNC_NAME,_,VALUE_SIZE,_,tev)
#include "elem_size_loop.h"
#undef MAKE_FUNC_NAME
	NULL
};

void CAT_TOKEN2(nark_, NARK_FUNC_NAME) // fun name
	(void* first, ptrdiff_t count, ptrdiff_t elem_size, ptrdiff_t field_offset, field_type_t field_type)
{
	...
	pf = (array_handler)get_heandler(CAT_TOKEN2(_S_tab_, NARK_FUNC_NAME), elem_size, field_type);
...
static const array_handler CAT_TOKEN2(Ptr_S_tab_, NARK_FUNC_NAME)[] =
{
