...
int
nark_trb_compare_less_tag(const struct trb_vtab* vtab,
							const struct trb_tree* tree,
							const void* x,
							const void* y)
{
	fprintf(stderr, "fatal: nark_trb_compare_less_tag is being called\n");
	abort();
	return 0;
}
						...
						const void* key)
{
	...
	if (nark_likely(NULL != tree->trb_root))
	{
					  ...
					  struct trb_path_arg* arg)
{
	...
	//	intptr_t l = 0;
		...
		if (nark_unlikely(k > TRB_MAX_HEIGHT)) {
...
trb_probe(const struct trb_vtab* vtab, struct trb_tree* tree, const void* data, int* existed)
{
	...
	if (nark_unlikely(NULL == n))
		return NULL;

	if (vtab->pf_data_copy)
	{
		if (nark_unlikely(!vtab->pf_data_copy(vtab, tree, vtab->data_offset + (char*)n, data)))
		{
...
int trb_erase(const struct trb_vtab* vtab, struct trb_tree* tree, const void* key)
{
	...
	if (nark_unlikely(NULL == tree->trb_root))
		return 0;
